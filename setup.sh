...
if [ -f /etc/debian_version ]; then
    sudo apt update
    sudo apt install -y zsh zoxide duf wireguard-tools vim bat tmux git nmap fzf
elif [ -f /etc/arch-release ]; then
    sudo pacman -S --noconfirm zsh zoxide duf wireguard-tools vim bat tmux git nmap fzf
else
...
sudo cp -f -v lukshandler restic-backup restic-clean /usr/local/bin/ 2>/dev/null || echo "Dotfiles not found, skipped copy"
sudo ln -s $PWD/resticenv /etc/resticenv
sudo chmod +x /usr/local/bin/{lukshandler,restic-backup,restic-clean}
sudo chown 1000:1000 /usr/local/bin/{lukshandler,restic-backup,restic-clean}
